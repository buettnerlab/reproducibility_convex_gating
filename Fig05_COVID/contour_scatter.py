...
def generate_hybrid_ticks_and_labels(lims, transform_func=None, inverse_transform_func=None, 
    ...
    if len(power_lims) > 0:
        ...
        min_log_power = max(1, int(np.log10(linear_threshold)))  # Start from 10^1 = 100
...
def arcsinh_transform(divisor=5.0):
    """
    Return forward and inverse functions for arcsinh(x / divisor).

    Parameters
    ----------
    divisor : float
        The scaling factor inside arcsinh. Default = 5.

    Returns
    -------
    tuple
        (forward, inverse)
        - forward(x): arcsinh(x / divisor)
        - inverse(y): divisor * sinh(y)
    """
    forward = lambda x: np.arcsinh(x / divisor)
    inverse = lambda y: divisor * np.sinh(y)
